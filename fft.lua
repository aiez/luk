...
local floor,sum              = L.floor, L.sum
local max, min, sqrt, exp    = L.max, L.min, L.sqrt, L.exp
...
local sd = function(num)
  ...
  return sqrt(max(0, num.m2) / (num.n - 1)) end

local welford = function(num, v, w)
  local n = num.n + w
  if n <= 0 then return Num() end
  local d = v - num.mu
  local mu = num.mu + w * d / n
  return Num(n, mu, num.m2 + w * d * (v - mu)) end
...
local cuts = function(data, rows, y)
  local u, ys = {}, (function() local _r={} for _, r in ipairs(rows) do _r[#_r+1]=y(r) end return _r end)()
  for _, at in ipairs(data.x) do
    ...
    for i, r in ipairs(rows) do
      local v = r[at]
      if v ~= "?" then
        local k = (c.symp and v) or floor(the.bins*norm(c, v))
        bins[k] = add(bins[k] or Num(), ys[i])
        tot     = add(tot, ys[i])
        hi[k] = c.symp and v or max(hi[k] or -BIG, v) end end
    local inner = (c.symp and cutsSyms or cutsNums)(
               bins, tot, hi, at)
...
grows = function(data, y, root, d)
  ...
  local u = {}
  if d < the.depth then
    local cs = (function() local _r={} for _, c in ipairs(cuts(data, data.rows, y))
             do if c.leaf.n > (#root.rows) ^ 0.33 then _r[#_r+1]=c end end return _r end)()
    if #cs > 0 then
      ...
      for bit, c in ipairs({cs[1], cs[#cs]}) do
        ...
        if #no > 0 then
          ...
          local bs = grows(Data(child), y, root, d+1)
...
local tune = function(cands, rows, y)
  local err = function(t)
    return sum((function() local _r={} for _, r in ipairs(rows) do _r[#_r+1]=abs(y(r) - predict(t, r))
           end return _r end)()) / #rows end
...
qty = function(v)
  if type(v) == "number" then
    if floor(v) == v then return floor(v) end
...
local test_main = function()
  ...
  local cands = (function() local _r={} for _, b in ipairs(grows(data, y, data)) do _r[#_r+1]=b.tree end return _r end)()
...
local test_trees = function()
  ...
  for i, bt in ipairs(trees) do
    local t = bt.tree
    local e = sum((function() local _r={} for _, r in ipairs(data.rows) do _r[#_r+1]=abs(y(r) - predict(t, r))
              end return _r end)()) / #data.rows
